...
export default function ConfiguracoesPage() {
  ...
  function update(index, field, value) {
    setPeriods(prev => prev.map((p, i) =>
      i === index ? { ...p, [field]: field === 'name' || field === 'description' || field === 'id' ? value : Number(value) } : p
    ));
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (data.ok) setMsg({ type: 'ok', text: 'Configurações salvas com sucesso!' });
  ...
  return (
    <section className="grid" style={{ gap: 20 }}>
      ...
      <div className="card">
        <h1 style={{ margin: 0 }}>Configurações de Metas</h1>
        <p className="muted" style={{ marginBottom: 0 }}>
          Defina metas de produtividade por período do mês. Ideal para a rotina contábil com quinzenas distintas.
        </p>
      </div>

      {activePeriod && (
        <div className="card" style={{ borderLeft: '4px solid #0f766e', background: '#f0fdfa' }}>
          <strong style={{ color: '#0f766e' }}>Período ativo hoje (dia {today}): {activePeriod.name}</strong>
          <p className="muted" style={{ margin: '4px 0 0' }}>
            Meta de foco: <strong>{activePeriod.focusTarget}%</strong> &nbsp;•&nbsp;
            Mínimo produtivo/dia: <strong>{activePeriod.minProductiveHoursPerDay}h</strong>
          </p>
        </div>
      )}
            ...
            <div>
              ...
              <div style={{ marginTop: 8, height: 8, background: '#e5e7eb', borderRadius: 99 }}>
                <div style={{
                  ...
                  width: `${Math.min(period.focusTarget, 100)}%`,
                  transition: 'width 0.3s'
                }} />
              </div>
              <div className="muted" style={{ fontSize: 12, marginTop: 4 }}>
                {period.focusTarget < 50 ? 'Meta baixa' : period.focusTarget < 75 ? 'Meta moderada' : 'Meta alta'}
              </div>
            </div>

            <div>
              <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>
                Mínimo produtivo por dia (h)
              </label>
              <input
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button
          onClick={save}
          disabled={saving}
          style={{ width: 'auto', padding: '10px 28px', fontSize: 15 }}
        >
